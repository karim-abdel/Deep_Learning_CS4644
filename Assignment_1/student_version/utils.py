...
def load_mnist_trainval():
    ...
    train_data = data[0:cutoff]
    train_label = label[0:cutoff]
    val_data = data[cutoff:]
    val_label = label[cutoff:]
    #############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle == True:
        pizza = list(zip(data, label))
        random.shuffle(pizza)
        data, label = zip(*pizza)
    ...
    for x,y in zip(data,label):
        batch_d.append(x)
        batch_l.append(y)
        if len(batch_d) == batch_size and len(batch_l) == batch_size:
            batched_data.append(batch_d)
            batched_label.append(batch_l)
            batch_d = []
            batch_l = []
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    #############################################################################
    ep1 = np.arange(len(train_loss_history))
    plt.plot(ep1,train_loss_history,'b', label = "train")
    plt.plot(ep1,valid_loss_history,'r', label = "valid")
    ...
    ep2 = np.arange(len(train_loss_history))
    plt.plot(ep2,train_acc_history,'b', label = "train")
    plt.plot(ep2,valid_acc_history,'r', label = "valid")
