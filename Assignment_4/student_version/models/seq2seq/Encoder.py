...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        self.lin1 = nn.Linear(self.encoder_hidden_size,self.encoder_hidden_size)
        self.relu = nn.ReLU()
        self.lin2 = nn.Linear(self.encoder_hidden_size, self.decoder_hidden_size)
    ...
    def forward(self, input):
        ...
        hid1 = self.lin1(hidden)
        hidRel = self.relu(hid1)
        hid2 = self.lin2(hidRel)
        hidden = torch.tanh(hid2)
