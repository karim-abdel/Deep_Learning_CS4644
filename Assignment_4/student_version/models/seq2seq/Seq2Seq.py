...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        voc = self.decoder.output_size

        outputs = torch.zeros(batch_size, out_seq_len, voc).to(self.device)

        g, hidden = self.encoder(source)

        dd = source[:, 0].unsqueeze(1)
        for t in range(out_seq_len):
            
            decod, hidden = self.decoder(dd, hidden, g, self.attention)
            
            outputs[:, t, :] = decod
            
            output=decod.argmax(dim=-1).unsqueeze(1)
