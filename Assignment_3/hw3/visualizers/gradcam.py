...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        ##############################################################################
        x, out = self.saved_tensors

        gr = torch.addcmul(torch.zeros(x.size()), y, (y > 0).type_as(y))

        gr = torch.addcmul(torch.zeros(x.size()), gr, (out > 0).type_as(gr))
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        target = output.gather(1, y_tensor.view(-1, 1))
        target.backward(torch.ones(target.size()))

        guided_backprop = X_tensor.grad.data.numpy().transpose((0, 2, 3, 1))
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        one_hot[torch.arange(output.shape[0]), y_tensor] = 1

        output.backward(gradient=one_hot, retain_graph=True)

        weights = torch.mean(self.gradient_value, axis=(2, 3)).unsqueeze(-1).unsqueeze(-1)
        cams = torch.sum(weights * self.activation_value, dim=1, keepdim=True)
        cam = np.maximum(cams.detach().numpy(), 0).squeeze(1)
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
