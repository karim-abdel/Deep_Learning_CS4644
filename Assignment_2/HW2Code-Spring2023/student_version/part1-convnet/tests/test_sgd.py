...
class TestSGD(unittest.TestCase):
    ...
    def test_sgd(self):
        ...
        expected_weights = genfromtxt('tests/sgd_weights/w.csv', delimiter=',')
        expected_bias = genfromtxt('tests/sgd_weights/b.csv', delimiter=',')

        self.assertAlmostEquals(np.sum(np.abs(expected_weights - model.modules[0].weight)), 0, places=5)
        self.assertAlmostEquals(np.sum(np.abs(expected_bias - model.modules[0].bias)), 0, places=5)